...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseMain() {
		...
		// <常量声明>
		auto errCD = Analyser::analyseConstantDeclaration();
		if (errCD.has_value())
			return errCD;
		// <变量声明>
		auto errVD = Analyser::analyseVariableDeclaration();
		if (errVD.has_value())
			return errVD;
		// <语句序列>
		auto errSS = Analyser::analyseStatementSequence();
		if (errSS.has_value())
			return errSS;
		return {};
	}
	...
	std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
		...
		while (true) {
			...
			// <标识符>
			auto identifer = nextToken();
			if (!identifer.has_value() || identifer.value().GetType() != TokenType::IDENTIFIER)
				return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrInvalidIdentifier);
			if(isDeclared(identifer.value().GetValueString()))
				return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrDuplicateDeclaration);
			// 变量可能没有初始化，仍然需要一次预读
			// '='
			next = nextToken();
			if (next.value().GetType() == TokenType::EQUAL_SIGN) {
				...
				if (err.has_value())
					return err;
				auto semicolon = nextToken();
				if (!semicolon.has_value() || semicolon.value().GetType() != TokenType::SEMICOLON)
					return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNoSemicolon);
				addVariable(identifer.value());
			}
			// ';'
			else{
				if (!next.has_value() || next.value().GetType() != TokenType::SEMICOLON)
					return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNoSemicolon);
				addUninitializedVariable(identifer.value());
	...
	std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t& out) {
		...
		if (next.value().GetType() == TokenType::PLUS_SIGN) {
			auto unsignedint = nextToken();
			if (unsignedint.value().GetType() != TokenType::UNSIGNED_INTEGER) {
				return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrAssignToConstant);
			}
			out = std::any_cast<int>(unsignedint.value().GetValue());
		}
			...
			out = std::any_cast<int>(unsignedint.value().GetValue()) * -1;
		}
	...
	std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
		...
		if (isConstant(id.value().GetValueString()))
		...
		if (!next.has_value() || next.value().GetType() != TokenType::SEMICOLON)
			return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNoSemicolon);
		if (isUninitializedVariable(id.value().GetValueString())) {
			_vars[id.value().GetValueString()]=getIndex(id.value().GetValueString());
			_uninitialized_vars.erase(id.value().GetValueString());
		}
		_instructions.emplace_back(Operation::STO, getIndex(id.value().GetValueString()));
		return {};
	}
